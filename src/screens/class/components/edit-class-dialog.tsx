...
interface FullState {
 ...
 class FullScreenDialog extends Component<FullProps, FullState> {
    ...
    handleSave = () => {
        ...
        this.cService.updateClassroom(body).subscribe(res => {
            //console.log(res);
    ...
    reset = () => {
        this.setState({title: '', saveText: 'Lưu', edit: false, allowWrite: true})
    }
    ...
    return (
      <div>
        <Dialog maxWidth='md' open={this.props.open} onClose={this.handleClose} TransitionComponent={Transition}>
          ...
          <MyInput
          label='Tên lớp học'
          onChange={(e:string)=>{this.setState({title: e, edit: true, disabled: false}); this.setMState('Lưu', false,false, '', true);}}
          fullWidth
          value={this.state.edit ? this.state.title : this.props.title}
